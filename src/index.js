...
async function handleDashboardApprove(request, env) {
  ...
  // send twice.
  const { token, draftId, editedBody, resend, variant } = await readJson(request);
  const tenantId = await verifyDashboardToken(env.SERVICEM8_APP_SECRET, token);
  ...
  try {
    await approveAndSendDraft(env, tenantId, draftId, editedBody, { resend: resend === true, variant: variant === "alt" ? "alt" : "main" });
    return json({ ok: true });
  } catch (err) {
